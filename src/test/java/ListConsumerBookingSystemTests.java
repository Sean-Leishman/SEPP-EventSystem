...
public class ListConsumerBookingSystemTests {
    @BeforeEach
    ...
    /**
     * Calls ListConsumerBookingsCommand and ensures outputted list is correct
     *
     * Register 3 different entertainment providers with 6 total events
     * Register 3 different consumers:
     *         - Consumer1 books 2 event
     *         - Consumer2 books 1 events
     *         - Consumer3 books 1 event
     *
     * Assert: the sizes of each consumers list is as listed previously
     *         log is correct
     *
     */
    @Test
    void listConsumerBookingTest() {
        ...
        loginConsumer1(controller);
        ListConsumerBookingsCommand cmd = new ListConsumerBookingsCommand();
        controller.runCommand(cmd);
        List<Booking> bookings1 = cmd.getResult();

        loginConsumer2(controller);
        ListConsumerBookingsCommand cmd1 = new ListConsumerBookingsCommand();
        controller.runCommand(cmd1);
        List<Booking> bookings2 = cmd1.getResult();

        loginConsumer3(controller);
        ListConsumerBookingsCommand cmd2 = new ListConsumerBookingsCommand();
        controller.runCommand(cmd2);
        List<Booking> bookings3 = cmd2.getResult();

        assertAll("Verify list of bookings for each consumer",
                () -> assertEquals(2, bookings1.size()),
                () -> assertEquals(1, bookings2.size()),
                () -> assertEquals(1,bookings3.size()),
                () -> assertEquals(3,Logger.getInstance().getLog().stream().filter(
                        log -> log.getResult().equals(
                                ListConsumerBookingsCommand.LogStatus.LIST_CONSUMER_BOOKINGS_SUCCESS.toString())).count())
                );
    }

    /**
     * Calls ListConsumerBookingsCommand when no user is logged in
     *
     * Register 3 different entertainment providers with 6 total events
     * Register 3 different consumers:
     *         - Consumer1 books 2 event
     *
     * Assert: the command result is null
     *         the log is correct
     *
     */
    @Test
    void verifyUserNotLoggedIn() {
        ...
        assertAll("Verify list of bookings for each consumer",
                () -> assertNull(bookings1),
                () -> assertTrue(Logger.getInstance().getLog().stream().anyMatch(
                        log -> log.getResult().equals(
                                ListConsumerBookingsCommand.LogStatus.LIST_CONSUMER_BOOKINGS_NOT_LOGGED_IN.toString())))

        );
    }

    /**
     * Calls ListConsumerBookingsCommand when the user logged in is not a Consumer
     *
     * Register 3 different entertainment providers with 6 total events
     * Register 3 different consumers:
     *         - Consumer1 books 2 event
     ...
     */
    @Test
    void verifyConsumerNotLoggedIn() {
        Controller controller = new Controller();
        ...
        assertAll("Verify list of bookings for each consumer",
                ...
                () -> assertTrue(Logger.getInstance().getLog().stream().anyMatch(
                        log -> log.getResult().equals(
                                ListConsumerBookingsCommand.LogStatus.LIST_CONSUMER_BOOKINGS_USER_NOT_CONSUMER.toString())))
